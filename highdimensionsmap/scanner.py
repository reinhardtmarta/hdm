...
class HDMScanner:
    ...
    def generate_dataset(self, n_samples=1, noise=0.05, drift=0.1):
        """Gera um conjunto de amostras em alta dimensão com estrutura temporal e ruído."""
        rng = np.random.default_rng(self.seed)
        base = rng.standard_normal((n_samples, self.input_dim))
        latent = rng.standard_normal((n_samples, self.latent_modes))

        projected = latent @ self.projection_matrix
        smooth = projected + drift * np.linspace(-1.0, 1.0, self.input_dim)
        noisy = smooth + noise * rng.standard_normal((n_samples, self.input_dim))
        dataset = noisy / np.linalg.norm(noisy, axis=1, keepdims=True)
        return dataset
    ...
    def query(self, dataset_signatures, query_vector, k=5):
